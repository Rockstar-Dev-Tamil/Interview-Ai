...
Takes the LangGraph state (must include `competency_map` and `covered_days`) and pulls a highly relevant interview question from the local FAISS index. It strictly prioritizes `Weak` and `Critical` areas.

### `evaluate_answer(question: str, answer: str) -> AnswerFeedback`
Uses Gemini 3.5 Flash to grade the candidate's raw answer. Returns a strict Pydantic object containing:
- `score` (0.0 to 1.0)
- `strengths` (List)
- `weaknesses` (List)
- `recommendation` (String)

### `validate_feedback_hallucination(question: str, answer: str, feedback: AnswerFeedback) -> bool`
